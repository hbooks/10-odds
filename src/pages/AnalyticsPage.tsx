...
function computeHippoStats(evals: HippoMarketEval[]) {
  ...
  const voids  = evals.filter(e => e.result === "void").length;
  const resolved = wins + losses;
  const winRate = resolved > 0 ? (wins / resolved) * 100 : 0;

  const marketMap: Record<string, { wins: number; losses: number; voids: number; total: number }> = {};
  evals.forEach(e => {
    const label = `${e.market} – ${e.selection}`;
    if (!marketMap[label]) marketMap[label] = { wins: 0, losses: 0, voids: 0, total: 0 };
    marketMap[label].total++;
    if (e.result === "won")  marketMap[label].wins++;
    else if (e.result === "lost") marketMap[label].losses++;
    else marketMap[label].voids++;
  });
  ...
  return { total, wins, losses, voids, winRate, marketBreakdown };
}
    ...
    <Layout>
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <SectionHeader
          ...
          subtitle="Performance of the 4 alternative markets Hippo selects for each prediction — voided picks excluded from win rate"
          accentColor={SAPPHIRE}
        />
          ...
          <>
            ...
            >
              <StatCard index={0} icon={Target}     label="Total Hippo Picks"
                ...
                sub={`${hippoStats.wins}W · ${hippoStats.losses}L · ${hippoStats.voids} Void`}
                accent={SAPPHIRE}
              />
              ...
              <StatCard index={2} icon={Flame}      label="Top Market"
                ...
                sub={`${hippoStats.marketBreakdown[0]?.winRate.toFixed(1)}% win rate`}
                accent={GOLD}
              />
              <StatCard index={3} icon={Zap}        label="Unique Pairs"
                value={hippoStats.marketBreakdown.length}
                sub="Market-selection pairs"
                accent={ROSE}
              />
            </motion.div>
            ...
            <ChartCard
              ...
              subtitle="Wins, losses, voids, and win rate per market-selection pair"
              icon={Layers}
              accentColor={GOLD}
              className="mb-4"
            >
                    ...
                    {hippoStats.marketBreakdown.map((row, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3 font-mono tabular-nums" style={{ color: "rgba(255,255,255,0.3)" }}>
                            {row.voids}
                          </td>
